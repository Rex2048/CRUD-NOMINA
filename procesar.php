...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $dias_trabajados = (int)$_POST['dias_trbajados'];
    $sueldo = (int)$_POST['sueldo'];
    //Falta agregar entradas del FRONT-END
    $dias_incapacidad = (int)$_POST['dias_incapacidad'];
    $horas_nocturnas_trabajadas = (int)$POST['horas_nocturnas_trabajadas'];
    $dias_dominicales = (int)$POST['dias_dominicales'];
    ...
    */
    ...
    if (!empty($nombre) && !empty($departamento) && !empty($cargo)
        ...
        $valor_hora_dominical = $hora_ordinaria * 1.90;
        ...
        */
        ...
        $nuevo_usuario = [
            'id' => uniqid(), // Genera un ID de texto único basado en microsegundos
            'nombre' => $nombre,
            'horas' => $horas,
            'pago_hora' => $pago_hora,
            'sueldo_total' => $sueldo_total // Almacenamos la variable calculada
        ];
